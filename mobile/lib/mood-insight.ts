import { miloMoodValence } from "@/lib/milo-mood"
...
export function proMoodInsightLine(moodHistory: { mood: number }[]): string | null {
  ...
  const last = miloMoodValence(moodHistory[moodHistory.length - 1]!.mood)
  const prev = miloMoodValence(moodHistory[moodHistory.length - 2]!.mood)
